...
public class Registrations {
    ...
    public JsonObject register(final Registration request) {
        this.tracer.info("registration arrived: " + request);
        final Registration registration = this.em.merge(request);
        this.tracer.info("registration stored: " + request);
        registration.setCalculator(this.priceCalculator::calculateTotal);
        this.tracer.info("price computed: " + registration.getTotalPrice());
        return convert(registration);
    }

    public Registration find(final long registrationId) {
        return this.em.find(Registration.class, registrationId);
    }

    public JsonObject findAsJson(final int registrationId) {
     ...
     * framework.
     */
    JsonObject convert(final Registration registration) {
        registration.setCalculator(this.priceCalculator::calculateTotal);
